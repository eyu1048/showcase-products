...
const Product = ({ data }) => {
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      const response = await axios.get("http://localhost:3001/admin/favorites");
  ...
  const handleFavorites = async (data) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:3001/admin/favorites",
  ...
  const removeFavorites = async (id) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:3001/admin/favorites/${id}`
      );
    ...
    <div className="product" key={id}>
      ...
      <div className="description">
        {data.files.map((file) => {
          ...
          return (
            <img
              src={`http://localhost:3001/admin/${file.path}`}
              onClick={handleClick}
            />
          );
        })}
